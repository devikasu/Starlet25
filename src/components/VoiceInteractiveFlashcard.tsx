...
const VoiceInteractiveFlashcard: React.FC<VoiceInteractiveFlashcardProps> = ({
  ...
  const handleVoiceCommand = (command: string) => {
    switch (command) {
      ...
      case 'previous':
        ...
        break;
      case 'repeat':
        voiceFlashcardSystem.readCurrentFlashcard();
        break;
      case 'answer':
        setIsAnswering(true);
        setUserAnswer('');
        setLastFeedback(null);
        voiceFlashcardSystem.startAnswerMode();
        break;
      case 'stop':
        onClose();
        break;
  ...
  const getSessionStats = () => {
    ...
    const correctAnswers = Array.from(session.userAnswers.values()).filter(answer => 
      answer.toLowerCase().includes('correct') || answer.length > 10
    ).length;
      ...
      <div className="bg-white rounded-lg p-6 max-w-4xl w-full mx-4 max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex justify-between items-center mb-4">
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-2">
              <div className={`w-3 h-3 rounded-full ${session?.isListening ? 'bg-green-500' : 'bg-gray-400'}`}></div>
              <span className="text-sm text-gray-600">
                {session?.isListening ? 'Listening' : session?.isSpeaking ? 'Speaking' : 'Ready'}
              </span>
            </div>
          </div>
          <button
        ...
        <div className="text-center mt-4">
          <p className="text-gray-600 text-sm">
            {session?.isListening ? '🎤 Listening for voice commands...' : 
